...
class SlidingWindowProcessor:
    ...
    def extract_nucleus_sequence(self, nucleus_id: int, timestamp: int) -> Optional[Dict[str, np.ndarray]]:
        """
    ...
    def _get_nucleus_bbox(self, nucleus_id: int, label_volume: np.ndarray) -> Optional[Tuple[slice, ...]]:
    ...
    def parallel_extract_nuclei(self, nuclei_list: List[int], timestamp: int) -> Dict[int, Optional[Dict[str, np.ndarray]]]:
        """
        ...
        if not self.config.use_parallel_processing:
            ...
            for nucleus_id in nuclei_list:
                results[nucleus_id] = self.extract_nucleus_sequence(nucleus_id, timestamp)
            return results
        ...
        with ThreadPoolExecutor(max_workers=self.config.num_workers) as executor:
            future_to_nucleus = {
                executor.submit(self.extract_nucleus_sequence, nucleus_id, timestamp): nucleus_id
                for nucleus_id in nuclei_list
            }
    ...
    def format_results(self, timestamp: int, predictions: Dict[int, Tuple[int, str, float]]) -> Dict[str, Any]:
        """
        ...
        with open(predictions_file, 'w') as f:
            for i, timestamp in enumerate(self.frame_indices):
                ...
                if processed_tensors:
                    batch_predictions = model_engine.batch_predict_gpu(processed_tensors)
                ...
                print(f"📊 Frame {timestamp:3d}/{self.frame_indices[-1]} | "
