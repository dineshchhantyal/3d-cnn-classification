...
def save_volume_slices(volume: np.ndarray, title: str, output_path: str, 
                      cmap: str = 'gray', is_binary: bool = False):
...
def save_segmentation_overlay(raw_volume: np.ndarray, mask_volume: np.ndarray, 
                            output_path: str):
...
def save_preprocessing_comparison(original_volumes: dict, processed_volumes: dict, 
                                output_path: str):
    """Save before/after comparison of preprocessing steps."""
    time_points = ['t-1', 't', 't+1']
    
    fig, axes = plt.subplots(2, 3, figsize=(18, 12))
    fig.suptitle('Preprocessing Comparison: Original vs Processed', fontsize=16, fontweight='bold')
    
    for i, tp in enumerate(time_points):
        if tp in original_volumes and original_volumes[tp] is not None:
            # Original volume (middle slice)
            orig_vol = original_volumes[tp]
            orig_slice = orig_vol[orig_vol.shape[0]//2]
            ...
            if i < len(processed_volumes):
                ...
                proc_slice = proc_vol[proc_vol.shape[0]//2]
            ...
            axes[0, i].text(0.5, 0.5, f'{tp}\nNot Available', 
                          ha='center', va='center', transform=axes[0, i].transAxes)
            axes[1, i].text(0.5, 0.5, f'{tp}\nNot Available', 
...
def save_model_activations(model: torch.nn.Module, input_tensor: torch.Tensor, 
    ...
    if activations:
        ...
        fig.suptitle('Model Activation Maps (Middle Slices)', fontsize=14, fontweight='bold')
        
        for i, (layer_name, activation) in enumerate(list(activations.items())[:n_layers]):
            ...
            if len(activation.shape) == 5:  # [batch, channels, depth, height, width]
                ...
                mean_activation = activation[0].mean(dim=0)[activation.shape[2]//2].numpy()
            else:
                continue
...
def save_prediction_analysis(predictions: torch.Tensor, pred_class: str, 
                           confidence: float, output_path: str):
    ...
    ax2.arrow(0, 0, needle_x*0.8, needle_y*0.8, head_width=0.05, 
              head_length=0.05, fc='red', ec='red', linewidth=3)
    
    ax2.set_xlim(-1.2, 1.2)
    ax2.set_ylim(-0.2, 1.2)
    ax2.set_aspect('equal')
    ax2.axis('off')
    ax2.set_title(f'Prediction Confidence: {confidence:.1%}', 
    ...
    for i, label in enumerate(['Very Low', 'Low', 'Medium', 'High', 'Very High']):
        ...
        ax2.text(x_label, y_label, label, ha='center', va='center', 
                fontsize=8, rotation=np.degrees(angle)-90 if angle > np.pi/2 else np.degrees(angle))
...
def generate_summary_report(dirs: dict, sample_name: str, pred_class: str, 
                          confidence: float, stats_file: str):
