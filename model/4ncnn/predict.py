...
def create_benchmark_summary(results, metadata):
    ...
    samples_with_known_labels = len([r for r in successful_results if r.get('true_class') is not None])
    samples_with_unknown_labels = total_processed - samples_with_known_labels
    correct_predictions = len([r for r in successful_results if r.get('correct') is True])
    incorrect_predictions = len([r for r in successful_results if r.get('correct') is False])
    
    overall_accuracy = (correct_predictions / (correct_predictions + incorrect_predictions) * 100) if (correct_predictions + incorrect_predictions) > 0 else 0
    ...
    # Confidence analysis
    all_confidences = [r.get('confidence', 0) for r in successful_results if r.get('confidence') is not None]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--analysis_level", choices=["basic", "detailed", "full"], default="detailed",
        ...
        if folder_paths:
            ...
            for i, folder_path in enumerate(folder_paths):
                ...
                try:
                    ...
                    correctness_icon = "✅" if is_correct is True else "❌" if is_correct is False else "❓"
                    ...
                    if true_class is not None:
                        print(f"   True: {true_class.upper()} → Predicted: {(pred_class or 'UNKNOWN').upper()} ({pred_confidence:.2%}) {correctness_icon}")
                    else:
                        print(f"   True: UNKNOWN → Predicted: {(pred_class or 'UNKNOWN').upper()} ({pred_confidence:.2%}) ❓")
            ...
            correct_predictions = len([r for r in results if 'error' not in r and r.get('correct') is True])
            incorrect_predictions = len([r for r in results if 'error' not in r and r.get('correct') is False])
            unknown_labels = len([r for r in results if 'error' not in r and r.get('correct') is None])
            ...
            for i, result in enumerate(results):
                if 'error' not in result:
                    ...
                    print(f"{i+1:2d}. {result['sample'][:35]:35} | True: {true_class.upper():12} → Pred: {pred_class.upper():12} ({confidence:.1%}) {icon}")
                else:
                ...
                if class_stats:
                    ...
                    for class_name in sorted(class_stats.keys()):
                        if class_name:  # Additional safety check
                            ...
                            print(f"  {class_name.upper():12}: {stats['correct']:2d}/{stats['total']:2d} ({class_accuracy:.1f}%)")
                        ...
                        if 'error' not in result:
                            ...
                            if os.path.exists(dirs['sample']):
                                report_path = generate_summary_report(
                                    dirs, sample_name, result['predicted_class'], 
                                    result['confidence'], 
                                    os.path.join(dirs['sample'], 'preprocessing', 'volume_statistics.json')
                                )
            ...
            if len(results) > 1:  # Only for batch processing
                try:
                    ...
                    metadata = {
                        ...
                        'model_timestamp': os.path.basename(os.path.dirname(args.model_path)) if 'training_outputs' in args.model_path else 'unknown',
            ...
            if args.volumes:
                sample_name = f"single_sample_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            ...
            if args.save_analysis:
                try:
                    ...
                    if os.path.exists(dirs['sample']):
                        report_path = generate_summary_report(
                            dirs, sample_name, pred_class, pred_confidence,
                            os.path.join(dirs['sample'], 'preprocessing', 'volume_statistics.json')
                        )
